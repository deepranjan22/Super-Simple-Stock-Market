...
public class GeometricMeanCalculatorServiceImplTest {

    @Autowired
    private GeometricMeanCalculatorServiceImpl geometricMeanCalculatorService;

    @Test
    public void shouldReturnZeroIfListIsEmpty() {
        final BigDecimal expected = BigDecimal.ZERO;
        BigDecimal actual=
                geometricMeanCalculatorService.calculate(Collections.emptyList());
    ...
    public void shouldReturnZeroIfListIsNull() {
        final BigDecimal expected = BigDecimal.ZERO;
        BigDecimal actual=
                geometricMeanCalculatorService.calculate(null);
        assertTrue(actual.compareTo(expected) == 0);
    }
    ...
    public void shouldReturn100AfterCalc() {
        ...
        BigDecimal actual = geometricMeanCalculatorService.calculate(trades).setScale(2, BigDecimal.ROUND_HALF_EVEN);
    ...
    public void shouldReturn26252AfterCalc() {
        BigDecimal expected = BigDecimal.valueOf(260.52);
        Trade trade1 = new Trade(1, TradeIndicator.BUY, BigDecimal.valueOf(100), 1);
